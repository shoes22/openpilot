...
class Planner(object):
  ...
  def update(self, sm, CP, VM, PP, live_map_data):
    ...
    map_age = cur_time - sm.rcv_time['liveMapData']
    ...
    if set_speed_limit_active and map_valid:
      if live_map_data.liveMapData.speedLimitValid:
        ...
        if speed_limit > 21.90496:
            v_speedlimit = speed_limit + float(11 * CV.MPH_TO_MS)
        else:
            v_speedlimit = speed_limit + float(6 * CV.MPH_TO_MS)
        if speed_limit != self.speed_limit_last:
            self.set_speed_override = True
            if speed_limit > 21.90496:
