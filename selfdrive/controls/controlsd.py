...
def calc_plan(CS, CP, VM, events, PL, LaC, LoC, v_cruise_kph, driver_status, geofence, speed_limit_last, rk, keep_this_frame, keep_this_speed):
  ...
  return plan, plan_ts, v_cruise_kph, speed_limit_last, update_speed_limit, keep_this_frame, keep_this_speed
...
def data_send(perception_state, plan, plan_ts, CS, CI, CP, VM, state, events, actuators, v_cruise_kph, rk, carstate,
              ...
              LaC, LoC, angle_offset, passive, start_time, set_follow_distance, update_speed_limit):
...
def controlsd_thread(gctx=None, rate=100, default_bias=0.):
  ...
  while True:
    ...
    plan, plan_ts, v_cruise_kph, speed_limit_last, update_speed_limit, keep_this_frame, keep_this_speed = calc_plan(CS, CP, VM, events, PL, LaC, LoC, v_cruise_kph, driver_status, geofence, speed_limit_last, rk, keep_this_frame, keep_this_speed)
    ...
    CC = data_send(PL.perception_state, plan, plan_ts, CS, CI, CP, VM, state, events, actuators, v_cruise_kph, rk, carstate, carcontrol,
                   live100, livempc, AM, driver_status, LaC, LoC, angle_offset, passive, start_time, set_follow_distance, update_speed_limit)
